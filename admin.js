...
function renderEditRecipe(){
  ...
  $('editSearch').oninput=showSearchResults;$('editSearchButton').onclick=showSearchResults;const categorySelect=$('editCategory');categorySelect.dataset.current='';categorySelect.onchange=()=>{const next=categorySelect.value,current=categorySelect.dataset.current;categorySelect.value=current;guardLeave(()=>{categorySelect.dataset.current=next;categorySelect.value=next;showCategoryResults(next)})};
...
function bindSortMoves(){
  document.querySelectorAll('[data-move]').forEach(button=>button.onclick=()=>{const row=button.closest('.sort-item'),sibling=button.dataset.move==='up'?row.previousElementSibling:row.nextElementSibling;if(!sibling)return;if(button.dataset.move==='up')row.parentElement.insertBefore(row,sibling);else row.parentElement.insertBefore(sibling,row);renumber();setDirty()});
  const list=document.querySelector('.sort-list');if(list&&window.Sortable)new window.Sortable(list,{handle:'.handle',animation:160,delay:120,delayOnTouchOnly:true,touchStartThreshold:4,onEnd:()=>{renumber();setDirty()}});
}
